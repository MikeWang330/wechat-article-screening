...
网页导航里有一个独立入口：`新品速递`。它面向饮料行业新品跟踪，用来把“新品线索”整理成可打开的本地报告。
...
1. 搜索和新品相关的公众号文章。
2. 过滤招聘、课程、资料包、纯快讯、活动预告、会议通知、低价值转载、不可访问链接和正文过短文章。
3. 验证微信原文链接。
4. 生成本地 HTML / Markdown / JSON。
5. 如果配置了 MinerU Token，会继续解析文章内容。
6. 如果配置了 LLM API Key，会尝试生成更完整的商业分析；没有 Key 时也会生成基础报告，并标注“高级分析未启用”。
...
没有可靠公开信息的地方会明确写成 `未找到可靠公开信息`、`基于公开信息推测` 或 `假设`，不会伪造成确定事实。

新品报告本地输出在：

```text
product_reports/
  20260707-120000-新品名称/
    report.md
    report.html
    report.json
```

`product_reports/` 是本地结果目录，不会上传到 GitHub。
...
```powershell
python new_product_monitor.py --cloud-safe
```

它会更新：

```text
data/new_product_discoveries.json
```

GitHub Actions 工作流位于：

```text
.github/workflows/new-product-monitor.yml
```
...
- 手动触发
- 生成结构化 JSON 数据
- 在配置 `FEISHU_WEBHOOK_URL` 后推送飞书摘要

注意：GitHub Actions 不能完成人工搜狗验证码。云端模式只处理已验证 URL、历史缓存或未来接入的无需人工验证搜索 API；需要 Chrome 验证的微信采集仍然使用本地网页完成。
